  ...
  return (
    <div className="p-8 bg-gray-50 dark:bg-primary min-h-screen">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-300 mt-1">Manage your customer base</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white dark:bg-secondary rounded-lg shadow p-6">
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Total Customers</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{customers.length}</p>
        </div>
        <div className="bg-white dark:bg-secondary rounded-lg shadow p-6">
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Active Customers</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">{customers.filter(c => c.status === 'Active').length}</p>
        </div>
        <div className="bg-white dark:bg-secondary rounded-lg shadow p-6">
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Avg Orders</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
            {(customers.reduce((acc, c) => acc + c.orders, 0) / customers.length).toFixed(1)}
          </p>
        </div>
        <div className="bg-white dark:bg-secondary rounded-lg shadow p-6">
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">Avg Spent</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white">
      ...
      <div className="bg-white dark:bg-secondary rounded-lg shadow">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {customers.map((customer) => (
                <tr key={customer.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{customer.orders}</td>
                  <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">${customer.spent.toFixed(2)}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">{customer.joined}</td>
                  <td className="px-6 py-4">
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-secondary text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700">
              Previous
            </button>
            <button className="px-4 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-secondary text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 dark:hover:bg-gray-700">
              Next
