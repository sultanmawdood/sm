    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-[1920px] mx-auto px-4 sm:px-8 lg:px-12 py-6 sm:py-12">
        <div className="flex justify-between items-center mb-8 sm:mb-12">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-primary">Cart</h1>
          {cart.length > 0 && (
            <button
              onClick={clearCart}
              className="text-sm text-secondary hover:text-red-600 transition-colors"
            >
              Clear Cart
            </button>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-4 sm:space-y-6">
              {cart.map((item) => (
                <div key={item.id} className="bg-light rounded-lg p-4 sm:p-6 transition-all hover:shadow-md">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1 min-w-0 pr-4">
                          <Link 
                            to={`/product/${item.id}`} 
                            className="text-primary font-semibold hover:text-secondary transition-colors text-base sm:text-lg block truncate"
                          >
                            {item.name}
                          </Link>
                          <p className="text-secondary text-xs sm:text-sm mt-1">{item.category}</p>
                        </div>
                        <p className="text-primary font-semibold text-base sm:text-lg whitespace-nowrap">
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center border-2 border-gray-300 rounded-lg overflow-hidden">
                          <button
                            ...
                            className="px-3 py-2 text-primary hover:bg-gray-200 transition-colors"
                          ...
                          <span className="px-4 py-2 text-primary font-semibold text-sm sm:text-base min-w-[3rem] text-center">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() => updateQuantity(item.id, item.quantity + 1)}
          ...
          <div className="lg:col-span-1">
            <div className="bg-light rounded-lg p-6 sticky top-24">
              <h2 className="text-xl sm:text-2xl font-bold text-primary mb-6">Order Summary</h2>
